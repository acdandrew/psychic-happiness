...
def get_array_generator(fname, batch_size=16, labels=[]):
    carray = bcolz.open(fname)
    limit = carray.shape[0]
    out = np.empty(shape=(batch_size,) + carray.shape[1:])
    while(True):
        i = 0
        for a in carray:
            out[i%batch_size] = a
            ...
            if i % batch_size == 0:
                if len(labels) > 0:
                    yield (out, labels[i-batch_size:i])
                else:
                    yield out
                out = np.empty(shape=(batch_size,) + carray.shape[1:])
